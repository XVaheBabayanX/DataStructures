...
BinaryTree::BinaryTree(const BinaryTree& other) : _Root(nullptr), _nodes(0) {
	if (other._Root != nullptr) {
		_Root = new Node(other._Root->_key, other._Root->_data);
		_nodes++;
...
BinaryTree::BinaryTree(const Node* root) : _Root(nullptr), _nodes(0) {
	if (root != nullptr) {
		_Root = new Node(root->_key, root->_data);
		_nodes++;
		copyTree(_Root, root);
	}
}
...
void BinaryTree::copyTree(Node* thisNode, const Node* otherNode) {
	if (otherNode->_leftChild != nullptr) {
		thisNode->_leftChild = new Node(otherNode->_leftChild->_key, otherNode->_leftChild->_data, thisNode);
		_nodes++;
		copyTree(thisNode->_leftChild, otherNode->_leftChild);
	}
	if (otherNode->_rightChild != nullptr) {
		thisNode->_rightChild = new Node(otherNode->_rightChild->_key, otherNode->_rightChild->_data, thisNode);
		_nodes++;
...
bool BinaryTree::Insert(const std::string Key, const double Data) {
	if (isEmpty(_Root)) {
...
bool BinaryTree::Delete(const std::string Key) {
...
Node* BinaryTree::Search(const std::string Key) const {
...
bool BinaryTree::Change(const std::string Key, const double Data) const {
...
std::string BinaryTree::MinimumKey() const {
	if (isEmpty(_Root)) return std::string(DefaultKey);
	return Minimum(_Root)->_key;
}

std::string BinaryTree::MaximumKey() const {
	if (isEmpty(_Root)) return std::string(DefaultKey);
...
void BinaryTree::RangeSearch(Node* node, std::string low, std::string high, std::vector<Node*>& result) {
	...
	std::string minValue = tree.Minimum(node)->_key;
	std::string maxValue = tree.Maximum(node)->_key;
...
Node* BinaryTree::buildBalancedTree(const std::vector<std::string>& keys, int start, int end) {
	if (start > end) {
		return nullptr;
	}
	...
	_nodes++;
	node->_leftChild = buildBalancedTree(keys, start, mid - 1);
	node->_rightChild = buildBalancedTree(keys, mid + 1, end);
