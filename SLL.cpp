...
void List::insertAtFront(const std::string name, const double value)
{
    ListNode* newNode = createNode(name, value);
    if (isEmpty())
...
void List::insertAtBack(const std::string name, const double value)
{
    ListNode* newNode = createNode(name, value);
    if (isEmpty())
        _firstPtr = _lastPtr = newNode;
...
bool List::insertAtIndex(const std::string name, const size_t index, const double value)
{
    if (index > _count)
        return false;
    ...
    if (isEmpty() || index == 0)
    {
        insertAtFront(name, value);
        return true;
    }

    if (index == _count)
    {
        insertAtBack(name, value);
        return true;
    }
